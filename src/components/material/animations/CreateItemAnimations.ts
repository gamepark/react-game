...
const createdItemIndexes = new WeakMap<object, Map<number, number>>()
...
function getInnerMap<T>(weakMap: WeakMap<object, Map<number, T>>, action: object): Map<number, T> {
...
export class CreateItemAnimations<P extends number = number, M extends number = number, L extends number = number>
  ...
  override getPreDuration(move: CreateItem<P, M, L>, context: MaterialGameAnimationContext<P, M, L>): number {
    ...
    getInnerMap(preQuantitySnapshots, context.action).set(key, items.map((item: any) => item.quantity))
    return 0
  }
    ...
    if (snapshot) {
      for (let i = 0; i < items.length; i++) {
        ...
        if (newQuantity > oldQuantity) {
          getInnerMap(createdItemIndexes, context.action).set(key, i)
          break
        }
      }
      ...
      getInnerMap(createdItemIndexes, context.action).set(key, rules.mutator(move.itemType).getItemCreationIndex(move.item))
    }
  ...
  isItemToAnimate(context: ItemContext<P, M, L>, animation: Animation<CreateItem<P, M, L>>): boolean {
    ...
    const createdIndex = createdItemIndexes.get(animation.action)?.get(key)
