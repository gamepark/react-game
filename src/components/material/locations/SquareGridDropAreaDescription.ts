...
import { getItemFromContext, ItemContext, Locator, SquareGridLocator } from '../../../locators'
import { DropAreaDescription } from './DropAreaDescription'
import { SimpleDropArea } from './SimpleDropArea'

export class SquareGridDropAreaDescription<P extends number = number, M extends number = number, L extends number = number, Id = any, R extends number = number, V extends number = number>
  ...
  getBestDropMove(moves: MaterialMove<P, M, L, R, V>[], location: Location<P, L>, context: ItemContext<P, M, L, R, V>, event: DragMoveEvent | DragEndEvent): MaterialMove<P, M, L, R, V> {
    ...
    if (itemRect && dropAreaRect) {
      ...
      if (isSquareGridLocator(locator)) {
        const bestMove = minBy(moves, move => {
          if (isMoveItem(move) && move.location.type === location.type) {
            ...
            const rotationDistance = Math.abs((Math.abs((move.location.rotation ?? 0) - (item.location.rotation ?? 0)) + 2) % 4 - 2)
            return distance
              + rotationDistance / 2
          }
          return Infinity
...
function isSquareGridLocator<P extends number = number, M extends number = number, L extends number = number, T = any, R extends number = number, V extends number = number>(
  locator: Locator<P, M, L, R, V>
): locator is SquareGridLocator<P, M, L, T, R, V> {
  return typeof (locator as SquareGridLocator<P, M, L, T, R, V>).getSquarePosition === 'function'
}
