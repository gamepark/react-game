...
export abstract class MobileMaterialDescription<P extends number = number, M extends number = number, L extends number = number, ItemId = any, R extends number = number, V extends number = number>
  extends MaterialDescription<P, M, L, ItemId, R, V> {
  isMobile = true

  canDrag(move: MaterialMove<P, M, L, R, V>, context: ItemContext<P, M, L, R, V>): boolean {
    if (isMoveItem(move)) {
  ...
  protected canDragToMove(move: MoveItem<P, M, L> | MoveItemsAtOnce<P, M, L>, context: ItemContext<P, M, L, R, V>): boolean {
  ...
  protected canDragToDelete(_move: DeleteItem<M>, context: ItemContext<P, M, L, R, V>): boolean {
