...
export abstract class PolyhexDescription<P extends number = number, M extends number = number, L extends number = number, ItemId = any, T = any, R extends number = number, V extends number = number>
   ...
   */
  getPolyhex(_item: MaterialItem<P, L, ItemId>, _context: ItemContext<P, M, L, R, V>): Polyhex<T> {
    return this.polyhex
  }
}

export function isPolyhexDescription<P extends number = number, M extends number = number, L extends number = number, ItemId = any, R extends number = number, V extends number = number>(
  description: ComponentDescription<ItemId>
): description is PolyhexDescription<P, M, L, ItemId, any, R, V> {
