...
export const ItemDisplay = forwardRef<HTMLDivElement, ItemDisplayProps>((
  ...
  const [locations, focusIndexes] = getLocationsWithFocus(item, itemContext, focus)
  const focusedLocations = useMemo(() => focusIndexes.map(index => locations[index]), [locations, focusIndexes])
  ...
  return <MaterialComponent ref={mergeRefs([ref, isFocused ? addFocusRef : undefined])}
                            ...
                            playDown={focus && !isFocused && !focusIndexes.length}
    ...
    {locations.map((location, index) => {
        const hasFocus = focusIndexes.includes(index)
